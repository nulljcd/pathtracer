...
impl Material {
    pub fn new(albedo: Vec3, metallic: f32) -> Self {
...
impl Environment {
    pub fn sample(&self, direction: Vec3) -> Vec3 {
        match self {
            Environment::Solid { albedo, strength } => albedo * strength,
...
impl Pathtracer {
    ...
    fn to_srgb(linear: Vec3) -> Vec3 {
        Vec3::new(
            Pathtracer::linear_to_srgb_component(linear.x),
            Pathtracer::linear_to_srgb_component(linear.y),
            Pathtracer::linear_to_srgb_component(linear.z),
        )
    }
        ...
        for _ in 0..num_samples {
            ...
            for _ in 0..num_bounces {
                ...
                if let Some((distance, hit_normal, material)) = scene.intersect_ray(&ray) {
                    ...
                    let diffuse_direction: Vec3 = (hit_normal + Pathtracer::random_direction(rng)).normalize();
                    let specular_direction: Vec3 = ray.direction - 2.0 * ray.direction.dot(hit_normal) * hit_normal;

                    ray.direction = diffuse_direction.lerp(specular_direction, material.metallic);
        ...
        buffer
            ...
            .for_each(|(y, row)| {
                ...
                for x in 0..resolution_x {
                    ...
                    let raw_color: Vec3 =
                        Pathtracer::trace_ray(&ray, scene, num_samples, num_bounces, &mut rng);
                    let exposed_color: Vec3 = raw_color * exposure_multiplier;
                    let mapped_color: Vec3 = Pathtracer::tone_map(exposed_color);
                    let final_color: Vec3 =
                        Pathtracer::to_srgb(mapped_color.clamp(Vec3::ZERO, Vec3::ONE));
...
fn main() {
    ...
    let objects: Vec<Object> = vec![
        ...
        Object::new(
            ...
            Material::new(Vec3::new(0.875, 0.725, 1.), 0.),
        ),
    ];
    ...
    let render_buffer: Vec<Vec3> = Pathtracer::render(resolution, &camera, &scene, 512, 6);
